 ...
 ******************************************************************************/
package org.usergrid.benchmark.commands.scheduled;
 ...
 * 
 */
public class ScheduledWriter extends ScheduledBase {

  protected static final Logger writeLogger = LoggerFactory.getLogger("write");
  
  protected static final Timer writesTimer = Metrics.newTimer(ScheduledWriter.class, "writes", TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
  ...
  private class QueueWorker implements Callable<Void> {
    ...
    public Void call() throws Exception {
      ...
      for (int i = 0; i < count; i++) {
        ...
        queue.schedule((long)(5000*Math.random()), TimeUnit.MILLISECONDS, event);
        timer.stop();
      }
