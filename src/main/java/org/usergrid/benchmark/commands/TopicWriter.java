 ...
 * 
 */
public class TopicWriter extends TopicBase {

  protected static final Logger writeLogger = LoggerFactory.getLogger("write");
  
  protected static final Timer writesTimer = Metrics.newTimer(TopicWriter.class, "writes", TimeUnit.MILLISECONDS, TimeUnit.SECONDS);
   ...
   */
  @Override
  protected void doWork(CommandLine line, Topic<TestEvent> topic, String hostName, int workers, int count)
      throws Exception {
    ...
    for (int i = 0; i < workers; i++) {
      ...
      futures.add(executor.submit(new QueueWorker(topic, hostName, i, count)));
    }
  ...
  private class QueueWorker implements Callable<Void> {
    ...
    private Topic<TestEvent> testTopic;
     ...
     * @param index
     */
    public QueueWorker(Topic<TestEvent> testTopic, String hostName, int threadIndex, int count) {
